...
// and the division by 2z is the derivative of z², to scale how much we adjust z by how quickly z² is changing. This general approach is called Newton's method. It works well for many functions but especially well for square root.)
...
func Sqrt(x float64) float64 {
	...
	for i:= 1; i < 10; i++ {
		z -= (z*z - x) / 2*z
		fmt.Printf("For i = %v the z value is %v\n", i, z)
	}
	return z
}
...
func main() {
	...
	sqrt := math.Sqrt(2)
	diff := sqrt - loopSqrt
	fmt.Printf("The function returns %g and the root from math is %g and the diff is %g", loopSqrt, sqrt, diff)
}
